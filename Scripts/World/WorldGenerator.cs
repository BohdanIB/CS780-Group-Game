...
{
	private const float ROAD_TRAVERSAL_COST = 1;
	private const float ROAD_CONSTRUCTION_COST = 0.3f;
	private const float ELEVATION_COST_SCALAR = 3;
	...
	public static GenericGrid<GroundTile> GenerateWorldAStar(Vector2I dimensions, Vector2I hubLocation)
	{
		...
		GenericGrid<GroundTile> newWorld =
			new GenericGrid<GroundTile>(dimensions.X, dimensions.Y,
				(g, x, y) =>
					new GroundTile(
						SelectTerrain(elevationNoise[x, y], humidityNoise[x, y], temperatureNoise[x, y], loadedTerrains),
						new Vector2I(x, y)));

		GridAStarPathfinder<GroundTile> pathfinder =
			new GridAStarPathfinder<GroundTile>(newWorld, (x, y) =>
			{
				...
				Dictionary<Vector2I, float> neighborCosts = new();
				...
				foreach (Vector2I coordinate in neighborPositions)
				{
					...
					{
						cost =
							(currentTile.HasRoadConnection()
								? ROAD_CONSTRUCTION_COST
								: ROAD_CONSTRUCTION_COST + (ELEVATION_COST_SCALAR * elevationNoise[currentTile.position.X, currentTile.position.Y]))
							+
							(nextTile.HasRoadConnection()
								? ROAD_CONSTRUCTION_COST
								: ROAD_CONSTRUCTION_COST + (ELEVATION_COST_SCALAR * elevationNoise[nextTile.position.X, nextTile.position.Y]));
		...
		List<Vector2I> targetPoints = new();
	...
	public static BiomeType[] LoadTerrains()
	{
		...
		List<BiomeType> loadedTerrains = new();
